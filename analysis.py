...
_THEME_RULES: List[tuple[str, str]] = [
    ...
    ("cloud", "Cloud"), ("saas", "SaaS"), ("gaming", "Gaming"),
    ("renewable", "Renewables"), ("solar", "Renewables - Solar"), ("wind", "Renewables - Wind"),
    ("ev ", "Electric Vehicles"), ("electric vehicle", "Electric Vehicles"),
    ("cybersecurity", "Cybersecurity"),
    ("oil", "Oil & Gas"), ("gas", "Oil & Gas"),
    ("uranium", "Uranium"), ("fintech", "Fintech"),
]
...
def apply_filters(
    ...
    out = df.copy()
    if "Sector" in out.columns and selected_sector != "All":
        out = out[out["Sector"].astype(str) == selected_sector]
    if "Industry Group" in out.columns and selected_industry_group != "All":
        out = out[out["Industry Group"].astype(str) == selected_industry_group]
    if "Industry" in out.columns and selected_industry != "All":
        out = out[out["Industry"].astype(str) == selected_industry]
    return out.reset_index(drop=True)

def fetch_taxonomy_only(symbols: Iterable[str]) -> pd.DataFrame:
    syms = _as_list(symbols)
    if not syms:
        df = pd.DataFrame(columns=["Symbol", "Sector", "Industry Group", "Industry", "Theme"])
    ...
    for s in syms:
        ...
        rows.append({
            ...
            "Sector": sector,
            "Industry": industry,
            "Industry Group": _infer_industry_group(industry),
